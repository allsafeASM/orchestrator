...
class ScanContext:
    ...
    def get_blob_path(self, task: str, extension: str, path_type: str = "out") -> str:
        ...
        if path_type == "out":
            return f"scans/{self.domain}-{self.scan_id}/{task}/out/final_out.{extension}"
        elif path_type == "in":
            return f"scans/{self.domain}-{self.scan_id}/{task}/in/input.txt"
        else:
            raise ValueError(f"Invalid path_type: {path_type}")
    
    def get_chunk_path(self, task: str, chunk_index: int) -> str:
        """Generate chunk blob path for this scan context"""
        return f"{self.domain}-{self.scan_id}/{task}/in/chunk_{chunk_index:04d}.txt" 